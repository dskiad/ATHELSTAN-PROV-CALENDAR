...
function setup(options = {}) {
  ...
  const fetch = async (url, settings) => {
    ...
    if (url.startsWith('https://api.github.com/repos/dskiad/ATHELSTAN-PROV-CALENDAR/contents/calendar-data.js')) {
      if (settings.method !== 'PUT') return json({ sha: 'current-file-sha' }, options.readStatus || 200);
  ...
  const sandbox = vm.createContext({ fetch, AbortController, sessionStorage, URLSearchParams, TextEncoder, btoa, Date: Clock, location,
...
test('token connection verifies identity, clears the field, and can be disconnected', async () => {
  ...
  assert.equal(s.requests.filter(r => r.method === 'PUT').length, 0);
...
test('direct save uses the current file SHA and waits for matching live data', async () => {
  ...
  const payload = JSON.parse(s.requests.find(r => r.method === 'PUT').body);
...
test('repeated clicks do not send duplicate saves', async () => {
  ...
  assert.equal(s.requests.filter(r => r.method === 'PUT').length, 1);
